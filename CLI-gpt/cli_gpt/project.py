...
from .errors import LoginRequired, PageStructureChanged, ProjectAccessFailed
...
def extract_project_id(url: str) -> str:
    try:
        ...
        raise ProjectAccessFailed("The project URL is malformed.") from exc
    project_id = next((part for part in parts if part.startswith("g-p-")), "")
    if not project_id:
...
({ messageSelectors, titleSelectors, exclusions }) => {
  ...
  return { recognized: messages.length > 0, title, messages };
}
"""
