...
class CliTests(unittest.TestCase):
    def test_argument_parsing(self):
        ...
        self.assertEqual(
            (send.command, send.chat_url, send.prompt),
            (
                "send",
    ...
    def test_setup_command(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            stdout = io.StringIO()
            with (
                patch("cli_gpt.cli.interactive_setup") as setup,
                patch("cli_gpt.cli.save_project_url") as save,
    ...
    def test_expected_error_is_friendly_without_traceback(self):
        stderr = io.StringIO()
        with (
            patch("cli_gpt.cli.continue_chat", side_effect=InvalidChatUrl("bad URL")),
    ...
    def test_exception_mapping(self):
        self.assertIn(
            "Prompt box not found", friendly_error(PromptBoxNotFound("changed"))
        )
